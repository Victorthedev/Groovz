...
async function createPlatformPlaylist(
  ...
  if (platform === 'spotify') return createSpotifyExport(trackIds, userId, suggestedName)
...
async function createSpotifyExport(
  ...
  const { id: playlistId, url } = await withRefresh(t =>
    createSpotifyPlaylist(suggestedName ?? 'Groovz Mix', 'Generated by Groovz', spotifyUserId, t),
  )
